...
def get_feature_selection_model_from_name(type_of_estimator, model_name):
    model_map = {
        'classifier': {
            'SelectFromModel': SelectFromModel(RandomForestClassifier(n_jobs=-1, max_depth=10, n_estimators=100), threshold='20*mean'),
            'RFECV': RFECV(estimator=RandomForestClassifier(n_jobs=-1), step=0.1),
            'GenericUnivariateSelect': GenericUnivariateSelect(),
            'KeepAll': 'KeepAll'
        },
        'regressor': {
            'SelectFromModel': SelectFromModel(RandomForestRegressor(n_jobs=-1, max_depth=10, n_estimators=100), threshold='0.7*mean'),
...
class FeatureSelectionTransformer(BaseEstimator, TransformerMixin):
        ...
        if self.selector == 'KeepAll':
            ...
            if self.feature_selection_model == 'SelectFromModel':
                ...
                if self.type_of_estimator == 'regressor':
                    self.estimator = RandomForestRegressor(n_jobs=-1, max_depth=10, n_estimators=100)
                else:
                    self.estimator = RandomForestClassifier(n_jobs=-1, max_depth=10, n_estimators=100)
    ...
    def transform(self, X, y=None):
        ...
        if scipy.sparse.issparse(X):
            ...
            return X.tocsr(), self.index_mask
